#!/usr/bin/env python3
"""
RQ4: Ecological Validity — Demographic Conditions (VP Gen-Prob)

Same as RQ1/ecological.py but adds a system-level persona prompt for each
demographic condition (Gender, Age, Political, Education × A/B).

For each model × condition, the system prompt is included in the chat
template so the LLM adopts the specified persona when computing
log-probabilities over VP scenario responses.

Usage:
    python RQ4/ecological_circumstance.py

Results:
    results/RQ4/ecological_circumstance/{condition_slug}/{model_slug}.json
"""
...
def _result_path(condition_slug: str, model_name: str) -> Path:
    return (
        ROOT / "results" / "RQ4" / "ecological_circumstance"
        / condition_slug / f"{model_slug(model_name)}.json"
    )


def save_result(result: dict, condition_slug: str, model_name: str):
    out_path = _result_path(condition_slug, model_name)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(result, indent=2, ensure_ascii=False))
    log(f"  => Saved: {out_path.relative_to(ROOT)}")


def _load_existing_result(condition_slug: str, model_name: str) -> dict | None:
    path = _result_path(condition_slug, model_name)
    if not path.exists():
        return None
    try:
...
def run_vp_batch(
    ...
    for idx in batch_indices:
        ...
        log(f"  [{idx + 1}/{total}] portrait_id={pid} ({ptype}) condition={condition_slug}")
        ...
        for out in scenario["outputs"]:
            ...
            if lp_data.get("error"):
                ...
                if consecutive_fails >= CONSECUTIVE_FAIL_LIMIT:
                    save_result(result, condition_slug, model_name)
...
def main():
    log("=" * 60)
    log("  RQ4: Ecological Validity — Demographic Conditions")
    log("=" * 60)
    ...
    for model_cfg in MODELS:
        ...
        all_done = all(
            _result_path(cond, model_name).exists()
            for cond in CONDITIONS
        )
        if all_done:
            log(f"SKIP {model_name}: all condition results exist.")
            continue
        ...
        for attempt in range(SERVER_LAUNCH_RETRIES):
            try:
                ...
                for cond_slug, sys_prompt in CONDITIONS.items():
                    existing = _load_existing_result(cond_slug, model_name)
                    done_pids: set[int] = set()
                    if existing:
                        done_pids = {r["portrait_id"] for r in existing.get("results", [])}
                        log(f"  Resuming {cond_slug}: {len(done_pids)}/{len(scenarios)} done")
                    ...
                    if not remaining:
                        log(f"  SKIP {cond_slug}: all {len(scenarios)} scenarios done")
                        continue
