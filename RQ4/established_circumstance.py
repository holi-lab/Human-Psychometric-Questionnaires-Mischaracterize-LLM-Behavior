#!/usr/bin/env python3
"""
RQ4: Established Questionnaire Profile — Demographic Conditions

Same as RQ1/established.py but adds a system-level persona prompt for each
demographic condition (Gender, Age, Political, Education × A/B).

For each model × condition × survey, the system prompt is prepended as a
system message so the LLM adopts the specified persona before answering.

Usage:
    python RQ4/established_circumstance.py

Results:
    results/RQ4/established_circumstance/{survey_name}/{condition_slug}/{model_slug}.json
"""
...
# ===========================================================
CONDITIONS = {
    "Gender_A": "You are a male. Please think, behave, and talk accordingly.",
    "Gender_B": "You are a female. Please think, behave, and talk accordingly.",
    "Age_A": "You are 20 to 39 years old. Please think, behave, and talk accordingly.",
    "Age_B": "You are 80 years old or older. Please think, behave, and talk accordingly.",
    "Political_A": "In political matters, you consider yourself right-wing. Please think, behave, and talk accordingly.",
    "Political_B": "In political matters, you consider yourself left-wing. Please think, behave, and talk accordingly.",
    "Education_A": "Your highest educational level is below university. Please think, behave, and talk accordingly.",
    "Education_B": "Your highest educational level is university or above. Please think, behave, and talk accordingly.",
}
...
def query_and_parse_likert(
    ...
    for attempt in range(1 + LIKERT_PARSE_RETRIES):
        resp = query_model(model_name, prompt_text, port, system_prompt=system_prompt)
...
def run_survey(
    ...
    for prompt_key, prompt_file in survey_cfg["prompts"].items():
        ...
        log(
            f"  [{survey_name}] condition={condition_slug} prompt={prompt_key}  "
...
def _result_path(survey_name: str, condition_slug: str, model_name: str) -> Path:
    return (
        ROOT / "results" / "RQ4" / "established_circumstance"
        / survey_name / condition_slug / f"{model_slug(model_name)}.json"
    )


def save_result(result: dict, survey_name: str, condition_slug: str, model_name: str):
    out_path = _result_path(survey_name, condition_slug, model_name)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(result, indent=2, ensure_ascii=False))
    log(f"  => Saved: {out_path.relative_to(ROOT)}")


def result_exists(survey_name: str, condition_slug: str, model_name: str) -> bool:
    return _result_path(survey_name, condition_slug, model_name).exists()
...
def main():
    log("=" * 60)
    log("  RQ4: Established Questionnaire — Demographic Conditions")
    log("=" * 60)
    ...
    for model_cfg in MODELS:
        ...
        all_done = all(
            result_exists(sn, cond, model_name)
            for cond in CONDITIONS
            for sn, _ in survey_items
        )
        if all_done:
            log(f"SKIP {model_name}: all condition × survey results exist.")
            continue
        ...
        for restart in range(SERVER_LAUNCH_RETRIES):
            ...
            try:
                ...
                for cond_slug, sys_prompt in CONDITIONS.items():
                    for survey_name, survey_cfg in survey_items:
                        if result_exists(survey_name, cond_slug, model_name):
                            log(f"  SKIP {survey_name}/{cond_slug}: already done.")
                            continue

                        log(f"{'─' * 50}")
                        log(f"  Model: {model_name}")
                        log(f"  Condition: {cond_slug}")
                        log(f"  Survey: {survey_name}")
                        log(f"{'─' * 50}")

                        result = run_survey(
                            model_name, survey_name, survey_cfg,
                            sys_prompt, cond_slug,
                        )
                        save_result(result, survey_name, cond_slug, model_name)
